...
?>
<?php
    $host = "mysql-masterbased.alwaysdata.net";
    $user_mysql = "232014";
    $password_mysql = "Kaka123.";
    $database = "masterbased_masterbased";
...
<center><img src="https://gifimage.net/wp-content/uploads/2017/07/anime-meme-gif-11.gif"/></center><br/><br/><br/><br/><br/>
<?php
            $user_agent = $_SERVER["HTTP_USER_AGENT"];

            if(!empty($_SERVER['HTTP_USER_AGENT'])){
                if(!preg_match_all("/\b(union|concat|group_concat|select|from|order|by|column_name|table_name|information_schema\\.tables|information_schema\\.columns)\b|\\(\\)|\'|--/i", $_SERVER['HTTP_USER_AGENT'])){
                    $query = "SELECT id, username FROM users WHERE id = " . $user_agent . " AND 1=2";
                    $succ = mysqli_query($conn, $query) or die("
                        <style>
                        body {
                        color:#00D3FF;
                        ...
                        <center><h4 style='color:red'><u>User-agent doesn't exist</u></h4>" . $_SERVER["HTTP_USER_AGENT"] . "</center>"

                    );
                    $rank = 1;
                    if (mysqli_num_rows($succ)) {
                        while ($row = mysqli_fetch_array($succ)) {
                            echo "<center><h4 style='color:white'>{$row['username']} exists</h4></center><br><br>";

                            $rank++;
                        }
                    } else {
                        echo "<center><h4 style='color:white'>" . $_SERVER["HTTP_USER_AGENT"] . "</h4></center><br><br>";
                    }
                } else {
                    echo "<br><br><center><h3 style='color:red'>Waf Detected</h3></center><br><br>";
                }
            } 
        ?>
...
<form method="POST">
    ...
    <p class="p-container">
        ...
        if(!empty(isset($_POST['submit']))){
            if(isset($_POST['flag']) && $_POST['flag'] == "t0r0nt0_ch4ll{M4st3r_SqL1_D0n3}" || $_POST['flag'] == "M4st3r_SqL1_D0n3"){
                ...
                if(in_array($search, $string)){
                    ...
                    echo "<center><h5 style='color:#20FF33'>+100 POINTS</h5></center>";
                    $host = "mysql-toronto.alwaysdata.net";
                    $user_mysql = "toronto";
                    $password_mysql = "Kaka123.";
                    ...
                    $sql = "UPDATE users SET pts = pts + 100 WHERE username = '" . $_SESSION["username"] . "'";
