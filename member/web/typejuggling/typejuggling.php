...
<center>
	<img src="https://image.noelshack.com/fichiers/2021/15/3/1618363901-heartyanimatedchrysomelid-size-restricted.gif"/><br/><br><br><br/><br><br><br/>
	<form method="GET">
		<input type="text" name="occurrence_x"/><br>
		<input type="text" name="occurrence_y"/><br>
	...
	<?php
		...
		if(!empty($_GET["submit"])){
			if(isset($_GET["occurrence_x"]) && isset($_GET["occurrence_y"])){
				if ($_GET["occurrence_x"] == $_GET["occurrence_y"]){
					print "<h3 style='color:red'>Doublon détecté</h3>";
				} else if(md5($_GET["occurrence_x"]) === md5($_GET["occurrence_y"])) {
					print $flag;
				} else {
					print "<h3 style='color:red'>Pas les mêmes hash</h3>";
				}
			}
	}
	?>
...
<form method="POST">
    ...
    <p class="p-container">
        ...
        if(!empty(isset($_POST['submit']))){
            if(isset($_POST['flag']) && $_POST['flag'] == "t0r0nt0_ch4ll{pHp_TyP3_C0nTr0l}" || $_POST['flag'] == "pHp_TyP3_C0nTr0l"){
                ...
                if(in_array($search, $string)){
                    ...
                    echo "<center><h5 style='color:#20FF33'>+45 POINTS</h5></center>";
                    $host = "mysql-toronto.alwaysdata.net";
                    $user_mysql = "toronto";
                    $password_mysql = "Kaka123.";
                    ...
                    $sql = "UPDATE users SET pts = pts + 45 WHERE username = '" . $_SESSION["username"] . "'";
