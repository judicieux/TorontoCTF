                                    ...
                                    <div class="col-md-4">
                                        <ul class="breadcrumb">
                                            <li class="breadcrumb-item">
                                                <a href="user.php?id=<?php htmlspecialchars_decode($_GET["id"]);?>"> <i class="fa fa-home"></i> </a>
                                            </li>
                                                    ...
                                                    <div class="card-header">
                                                        ...
                                                        <div id="accordion" role="tablist" aria-multiselectable="true">
                                                            <div class="accordion-panel">
                                                                ...
                                                                $sql = "SELECT * FROM users WHERE id = '".$bdd->real_escape_string($_GET['id'])."'";
                                                                $result = mysqli_query($bdd, $sql);
                                                                if (mysqli_num_rows($result)) {
                                                                    while ($row = mysqli_fetch_array($result)) {
                                                                        if(!empty($row['avatar'])){
                                                                ...
                                                                ?>
                                    <img src="avatars/<?php echo $row['avatar']; ?>" class="img-radius" alt="User-Profile-Image" width="115" height="115" style="object-fit: cover;"/>
                                                                <?php
echo "<br/><br/>";
                                                                } else {
																		echo "<u><p style='font-weight: bolder;'>Avatar</p></u><br/>";
                                                                ?>
                                                                <img class="d-flex align-self-center img-radius" src="avatars/default.png" alt="Generic placeholder image" width="115" height="115" />
                                                                <?php
echo "<br/><br/>";
                                                                }
                                                                ?>
															...
															<div class="accordion-heading" role="tab" id="headingOne">
                                                                    <h3 class="card-title accordion-title">
                                                                        ...
                                                                        aria-expanded="true" aria-controls="collapseOne">
                                                                        <?php
                                                                $lol = "SELECT * FROM users WHERE id = '".$bdd->real_escape_string($_GET['id'])."'";
															...
															<div class="accordion-heading" role="tab" id="headingOne">
                                                                ...
                                                                if (mysqli_num_rows($result2)) {
                                                                    while ($row = mysqli_fetch_array($result2)) {
																		...
																		if($row["id"] == 1){
																			echo "<img src='https://img.shields.io/badge/1KfJxfRp3xXpmHEXEen9ZWcdPyknsEHrXY-000000?style=for-the-badge&logo=bitcoin&logoColor=white'><br/><br/>";
																		}																																		if($row["id"] == 179){
																			echo "<img src='https://img.shields.io/badge/8-8f76c4?style=for-the-badge&logoColor=white'><br/><br/>";
																		}
                                                                        if($row["id"] == 353){
                                                                            echo "<img src='https://img.shields.io/badge/informatic%20enthusiast-000000?style=for-the-badge&logoColor=white'><br/><br/>";
                                                                        }
                                                                        if($row["id"] == 203){
                                                                            echo "<img src='https://img.shields.io/badge/akerman%20pro%20ax0r-F99713?style=for-the-badge&logoColor=white'><br/><br/>";
                                                                        }
																		if($row["pts"] <= 50){
	...
	<h3 class="card-title accordion-title">
		<a class="accordion-msg waves-effect waves-dark" data-toggle="collapse"
		   ...
		   aria-expanded="true" aria-controls="collapseOne">
			...
			<?php
			$sql = "SELECT respects FROM users WHERE id = '" . $bdd->real_escape_string($_GET["id"]) . "'";
		...
		<?php
		$sql = "SELECT username FROM users WHERE id = '" . $bdd->real_escape_string($_GET["id"]) . "'";
		$query = mysqli_query($bdd, $sql);
		if(mysqli_num_rows($query)){
			while($row = mysqli_fetch_array($query)){
				if(strpos(file_get_contents("respects/" . $row["username"] . ".txt"), $_SESSION["username"]) !== false && $_SESSION["username"] !== $row["username"]){
					echo '<div class="accordion-heading" role="tab" id="headingOne">
																	<h3 class="card-title accordion-title">
																		<a class="accordion-msg waves-effect waves-dark" data-toggle="collapse"
																		   data-parent="#accordion" href="#collapseOne"
																		   aria-expanded="true" aria-controls="collapseOne">    
...
<form method="POST">
    <p class="p-container"><button style="position:relative;left:30px;top:35px" name="respect" class="button-already-cliqued" disabled><span class="text">Respect</span><span class="icon"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64" aria-labelledby="title" aria-describedby="desc" role="img" xmlns:xlink="http://www.w3.org/1999/xlink"><path data-name="layer1" fill="none" stroke="#FFFFFF" stroke-miterlimit="10" stroke-width="5" d="M2 30l21 22 39-40" stroke-linejoin="round" stroke-linecap="round"></path></svg></span></button>';
					echo '<button style="position:relative;left:200px;top:-15px" name="unrespect" class="noselect"><span class="text">Unrespect</span><span class="icon"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path d="M24 20.188l-8.315-8.209 8.2-8.282-3.697-3.697-8.212 8.318-8.31-8.203-3.666 3.666 8.321 8.24-8.206 8.313 3.666 3.666 8.237-8.318 8.285 8.203z"/></svg></span></button></p>
	<br/>
</form>';
				} elseif(strpos(file_get_contents("respects/" . $row["username"] . ".txt"), $_SESSION["username"]) !== true && $_SESSION["username"] !== $row["username"]) {
					echo '<div class="accordion-heading" role="tab" id="headingOne">
																	<h3 class="card-title accordion-title">
																		<a class="accordion-msg waves-effect waves-dark" data-toggle="collapse"
																		   data-parent="#accordion" href="#collapseOne"
...
<form method="POST">
    <p class="p-container"><button style="position:relative;left:30px;top:35px" name="respect" class="respect-button"><span class="text">Respect</span><span class="icon"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64" aria-labelledby="title" aria-describedby="desc" role="img" xmlns:xlink="http://www.w3.org/1999/xlink"><path data-name="layer1" fill="none" stroke="#FFFFFF" stroke-miterlimit="10" stroke-width="5" d="M2 30l21 22 39-40" stroke-linejoin="round" stroke-linecap="round"></path></svg></span></button>';
					echo '<button  style="position:relative;left:200px;top:-15px" name="unrespect" class="button-already-cliqued" disabled><span class="text">Unrespect</span><span class="icon"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path d="M24 20.188l-8.315-8.209 8.2-8.282-3.697-3.697-8.212 8.318-8.31-8.203-3.666 3.666 8.321 8.24-8.206 8.313 3.666 3.666 8.237-8.318 8.285 8.203z"/></svg></span></button></p>
	<br/>
</form>';
				}
			}
		...
		if(mysqli_num_rows($query)){
			while($row = mysqli_fetch_array($query)){
				if(isset($_POST["respect"]) && $_POST["respect"] !== NULL && $row["username"] !== $_SESSION["username"]){
					$sql = "SELECT * FROM users WHERE id = '" . $bdd->real_escape_string($_GET["id"]) . "'";
					$query = mysqli_query($bdd, $sql);
					if(mysqli_num_rows($query)){
						while($row = mysqli_fetch_array($query)){
							if(!file_exists("respects/" . $row["username"] . ".txt")){
								$create_file = fopen('respects/' . $row["username"] . ".txt", "w");
								fwrite($create_file, $_SESSION["username"] . "\n");
								fclose($create_file);
								$push_respect = "UPDATE users SET respects = respects + 1 WHERE id = '" . $bdd->real_escape_string($_GET["id"]) . "'";
								$query = mysqli_query($bdd, $push_respect);
								echo "<script>location.reload();</script>";
							} elseif(strpos(file_get_contents("respects/" . $row["username"] . ".txt"), $_SESSION["username"]) === false){
								$verify_file = fopen("respects/" . $row["username"] . ".txt", "a+");
								fwrite($verify_file, $_SESSION["username"] . "\n");
								$push_respect = "UPDATE users SET respects = respects + 1 WHERE id = '" . $bdd->real_escape_string($_GET["id"]) . "'";
								$query = mysqli_query($bdd, $push_respect);
								echo "<script>location.reload();</script>";
							} 
				...
				if(isset($_POST["unrespect"]) && $_POST["unrespect"] !== NULL && $row["username"] !== $_SESSION["username"]){
					$sql = "SELECT * FROM users WHERE id = '" . $bdd->real_escape_string($_GET["id"]) . "'";
					$query = mysqli_query($bdd, $sql);
					if(mysqli_num_rows($query)){
						while($row = mysqli_fetch_array($query)){
							if(strpos(file_get_contents("respects/" . $row["username"] . ".txt"), $_SESSION["username"]) !== false){
								$sql = "UPDATE users SET respects = respects - 1 WHERE id = '" . $bdd->real_escape_string($_GET["id"]) . "'";
								$query = mysqli_query($bdd, $sql);
								$lines  = file("respects/" . $row["username"] . ".txt");
								$search = $_SESSION["username"];
								foreach($lines as $line) {
									if(strpos($line, $search) === false) {
										$result = str_replace($_SESSION["username"], "", $line);
									}
								}
								file_put_contents("respects/" . $row["username"] . ".txt", $result);
								echo "<script>location.reload();</script>";
							}
						}
					}
				...
				foreach($contents as $line) {
					...
					if(mysqli_num_rows($query)){
						while($row = mysqli_fetch_array($query)){
							if(!empty($row['avatar'])){
		?>
        <img src="avatars/<?php echo $row['avatar']; ?>" class="img-radius" alt="User-Profile-Image" width="25px" height="25px" style='object-fit: cover;'/>
		<?php
