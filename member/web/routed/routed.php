...
<?php
    $host = "mysql-routedbased.alwaysdata.net";
    $user_mysql = "232288";
    $password_mysql = "Kaka123.";
    $database = "routedbased_routedbased";
...
<center><img src="https://static.tumblr.com/8c05491d94ccddbfee6d1da6b71f9bf4/kejlp19/Nw0otbl50/tumblr_static_tumblr_static__focused_v3.gif"/></center><br/><br/><br/><br/><br/>
        <?php
        if(!empty($_GET['id'])){
            $id = $_GET['id'];
            $query = "SELECT id, code FROM users WHERE id = '$id'";
            $succ = mysqli_query($conn, $query);
            if (mysqli_num_rows($succ)) {
                $row = mysqli_fetch_array($succ);
                $query2 = "SELECT username FROM users WHERE code = '" . $row['code'] . "'";
                $succ2 = mysqli_query($conn, $query2);
                if (mysqli_num_rows($succ2)) {
                    $row2 = mysqli_fetch_array($succ2);
                    echo "<center><h4 style='color:white'>" . $row2['username'] . "</h4></center>";
                }
            }
        }
...
<form method="POST">
    ...
    <p class="p-container">
        ...
        if(!empty(isset($_POST['submit']))){
            if(isset($_POST['flag']) && $_POST['flag'] == "t0r0nt0_ch4ll{R0Ut3D_L1k3_4_Pr0}" || $_POST['flag'] == "R0Ut3D_L1k3_4_Pr0"){
                ...
                if(in_array($search, $string)){
                    ...
                    echo "<center><h5 style='color:#20FF33'>+90 POINTS</h5></center>";
                    $host = "mysql-toronto.alwaysdata.net";
                    $user_mysql = "toronto";
                    $password_mysql = "Kaka123.";
                    ...
                    $sql = "UPDATE users SET pts = pts + 90 WHERE username = '" . $_SESSION["username"] . "'";
